...
This article presents a sample data analysis using methods described in two separate articles/tutorial papers [@masaki2010] [@distefano2009understanding]. To recap this is what we aim to achieve from this part of the analysis.

1. Explore the **empirical correlations** between our items; not just theorectical correlations (some questionnaire items might end up capturing none or more than one trait and we can't perform hypothesis testing with that item)
...
An **artificial dataset** containing 500 observations of 15 questions was generated to simulate a 15 question-long survey form conducted on (n=500) respondents. They are designed as if asking about two different themes (e.g. perceived severity of haze, knowledge of protective measures) on a 7 point Likert-type scale.
...
Questions: Q5 (0.83 on PC2 and -0.32 on PC6) Q15 (0.89 on PC6 and 0.39 on PC1) can be discarded based on this criteria.

***

```{r, echo=FALSE}
test <- test[,-c(5,15)] #remove Q5 and Q15 on test data set
```
...
random.polychor.pa(nrep=10, data.matrix=data.matrix(test), q.eigen = 0.99, r.seed = 1, diff.fact=FALSE, distr="NULL", comparison = "random", fit.pa=FALSE, print.all=FALSE)
#
```
...
- The remaining questions (Q4,6,13 & 14) were designed not be highly correlated with the the two factors but nonetheless did not meet the criteria to be discarded
...
```{r, echo=TRUE}
bartlett.scores <- factor.scores(data.matrix(test.final),f=test.final.fa,method="Bartlett")$scores
test.wscores <- cbind(test.final,bartlett.scores)
```
...
par(mfrow=c(1,2))
hist(test.wscores$Factor1, main="Factor 1", xlab="Bartlett Score", breaks=20)
hist(test.wscores$Factor2, main="Factor 2", xlab="Bartlett Score", breaks=20)
#
```

The scores are unfortunately not normal with the bogus data generated for this bogus analysis (Log transforming or Square root did not address this issue adequately). This assumption will have to be checked before performing further analysis.
